...
function activate(context) {
    ...
    const openPullRequestsWebviewCommand = vscode.commands.registerCommand('gitea.openPullRequestsWebview', async () => {
        try {
            const pullRequests = await giteaService.getPullRequests();
            const panel = vscode.window.createWebviewPanel('giteaPullRequests', 'Gitea Pull Requests', vscode.ViewColumn.One, {
                enableScripts: true,
    ...
    const openIssuesWebviewCommand = vscode.commands.registerCommand('gitea.openIssuesWebview', async () => {
        try {
            const issues = await giteaService.getIssues();
            const panel = vscode.window.createWebviewPanel('giteaIssues', 'Gitea Issues', vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    const listAllPullRequestsCommand = vscode.commands.registerCommand('gitea.listAllPullRequests', async () => {
        try {
            const pullRequests = await giteaService.getPullRequests();
            const panel = vscode.window.createWebviewPanel('giteaAllPullRequests', 'All Pull Requests', vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    const listAllIssuesCommand = vscode.commands.registerCommand('gitea.listAllIssues', async () => {
        try {
            const issues = await giteaService.getIssues();
            const panel = vscode.window.createWebviewPanel('giteaAllIssues', 'All Issues', vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    context.subscriptions.push(viewPullRequestsCommand, viewIssuesCommand, refreshPullRequestsCommand, refreshIssuesCommand, showPullRequestDetailsCommand, showIssueDetailsCommand, openPullRequestsWebviewCommand, openIssuesWebviewCommand, createNewPullRequestCommand, createNewIssueCommand, listAllPullRequestsCommand, listAllIssuesCommand);
}
exports.activate = activate;
async function showPullRequestsInMarkdown(pullRequests) {
...
async function showIssuesInMarkdown(issues) {
...
async function showPullRequestDetailsInMarkdown(pullRequest) {
...
async function showIssueDetailsInMarkdown(issue) {
...
function generatePullRequestsMarkdown(pullRequests) {
...
function generateIssuesMarkdown(issues) {
...
function generateSinglePullRequestMarkdown(pr) {
    const status = pr.mergeable ? '✅ Mergeable' : '❌ Not Mergeable';
    const state = pr.state === 'open' ? '🟢 Open' : '🔴 Closed';
...
function generateSingleIssueMarkdown(issue) {
    const state = issue.state === 'open' ? '🟢 Open' : '🔴 Closed';
...
function generatePullRequestsWebviewHtml(pullRequests) {
    return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
...
function generateIssuesWebviewHtml(issues) {
    return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
...
async function showPullRequestDetailsWebview(pullRequest, giteaService) {
    const panel = vscode.window.createWebviewPanel('giteaPRDetails', `PR #${pullRequest.number}: ${pullRequest.title}`, vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
...
async function showIssueDetailsWebview(issue, giteaService) {
    const panel = vscode.window.createWebviewPanel('giteaIssueDetails', `Issue #${issue.number}: ${issue.title}`, vscode.ViewColumn.One, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
...
function generatePullRequestDetailsHtml(pr) {
...
function generateAllPullRequestsHtml(pullRequests) {
    return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
...
function generateAllIssuesHtml(issues) {
    return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
