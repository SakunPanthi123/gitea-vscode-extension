...
export function activate(context: vscode.ExtensionContext) {
  ...
  const reactWebviewProvider = new ReactWebviewProvider(
  ...
  vscode.commands.executeCommand(
    "setContext",
    "giteaPullRequestsVisible",
    true
  );
  vscode.commands.executeCommand("setContext", "giteaIssuesVisible", true);

  // Register commands
  const viewPullRequestsCommand = vscode.commands.registerCommand(
  ...
  const viewIssuesCommand = vscode.commands.registerCommand(
    "gitea.viewIssues",
    async () => {
  ...
  const refreshPullRequestsCommand = vscode.commands.registerCommand(
    "gitea.refreshPullRequests",
    async () => {
      try {
        pullRequestProvider.refresh();
        vscode.window.showInformationMessage(
          "Pull requests refreshed successfully!"
        );
      } catch (error) {
  ...
  const refreshIssuesCommand = vscode.commands.registerCommand(
    "gitea.refreshIssues",
    async () => {
      try {
  ...
  const showPullRequestDetailsCommand = vscode.commands.registerCommand(
  ...
  const listAllPullRequestsCommand = vscode.commands.registerCommand(
    "gitea.listAllPullRequests",
    async () => {
      await reactWebviewProvider.showPullRequestsList();
    }
  );

  const listAllIssuesCommand = vscode.commands.registerCommand(
    "gitea.listAllIssues",
    async () => {
      await reactWebviewProvider.showIssuesList();
    }
