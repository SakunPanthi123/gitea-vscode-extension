      ...
      {data && data.length > 0 ? (
        <div className="space-y-4">
          {data.map((issue) => (
            <div
              key={issue.id}
              className="border border-vscode-border -lg p-4 hover:bg-gray-50 hover:bg-opacity-5 transition-colors"
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2 ml-4">
                  <button
                    onClick={() => handleShowDetails(issue)}
                    className="px-3 py-1 text-sm bg-blue-600 hover:bg-blue-700  transition-colors"
                  >
                    Details
                  </button>
                  <button
                    onClick={() => handleOpenExternal(issue.html_url)}
                    className="px-3 py-1 text-sm bg-gray-600 hover:bg-gray-700  transition-colors"
                  >
                    Open
                  </button>
                </div>
